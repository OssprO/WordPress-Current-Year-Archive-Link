...
/*
Plugin Name:  WordPress Current Year Archive Links
Plugin URI:   https://github.com/OssprO/current-year-archive-link
Description:  Adds a MetaBox to the Appearance > Menu page to add current year archive link
Version:      0.0.1
Author:       Osiel Hernández
Author URI:   https://github.com/OssprO/
Author Email: osspro@live.com.mx
Contributors: 
License:      GPLv3
...
Domain Path:  /lang/
	...
	Copyright 2015 Osiel Hernández
...
add_action( 'plugins_loaded', array( new Current_Year_Archive_Links, 'init' ) );

class Current_Year_Archive_Links {
	/**
	...
	/**
	 * Nonce Value
	 * @const \Current_Year_Archive_Links::NONCE
	 */
	const NONCE = 'cyear_nonce';

	/**
	 * ID of the custom metabox
	 * @const \Current_Year_Archive_Links::METABOXID
	 */
	const METABOXID = 'cyear-metabox';

	/**
	 * ID of the custom post type list items
	 * @const \Current_Year_Archive_Links::METABOXLISTID
	 */
	const METABOXLISTID = 'current-year-archive-checklist';
	 ...
	 * @return \Current_Year_Archive_Links
	 * @use \Current_Year_Archive_Links::enable() Add hooks, load domain
	 */
	public function init() {
		if ( ! $this->ininited ) {
			...
			* by calling $hptal = apply_filters( 'current_year_archive_links', NULL );
			* No singleton was harmed in the making of this plugin.
			*/
			add_filter( 'current_year_archive_links', array( $this, __FUNCTION__ ) );
		}
		return $this;
	}
	...
	private function enable( $path ) {
		...
		add_action( 'admin_init', array( $this, 'add_custom_meta_box' ), 20 );
		...
		add_action( "wp_ajax_" . self::NONCE, array( $this, 'ajax_add_year' ) );
	...
	public function disable() {
		if ( $this->ininited ) {
			...
			remove_action( 'admin_init', array( $this, 'add_custom_meta_box' ), 20 );
			...
			remove_action( "wp_ajax_" . self::NONCE, array( $this, 'ajax_add_year' ) );
	...
	public function custom_meta_box_markup($object) {
		...
		$html .= '<input type="submit"'. disabled( $nav_menu_selected_id, 0, false ) .' class="button-secondary
			  submit-add-to-menu right" value="'. esc_attr__( 'Add to Menu' ) .'" 
			  name="add-post-type-menu-item" id="submit-current-year-archives" />';
	...
	public function metabox_script( $hook ) {
		...
		wp_enqueue_script( 'cyear-ajax-script' );
	...
	public function ajax_add_year() {
		...
		$menu_item_data= array(
			 'menu-item-title'  => esc_attr( $years[0] )
			,'menu-item-type'   => 'cyear_archive'
			,'menu-item-object' => esc_attr( $years[0] )
			,'menu-item-url'    => get_year_link($years[0])
		);
	...
	public function is_allowed() {
		...
		empty( $years['years'] ) AND exit;
		// return years if chosen
		return array_values( $years['years'] );
	}
	...
	public function setup_archive_item( $menu_item ) {
		if ( $menu_item->type !== 'cyear_archive' )
			return $menu_item;

		$year = $menu_item->object;
		$menu_item->type_label = __( 'Archive' );
		$menu_item->url = get_year_link($year);
